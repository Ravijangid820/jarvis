...
| 2a | **Device-command understanding is hardcoded** — the fast-paths match fixed verb lists/regexes (`intents.py`), so paraphrases and indirect requests ("it's boiling in here", "make it dark") don't act; they get a clarification or a plain LLM answer. Adding a phrasing means editing a regex. | Jarvis feels scripted for vocabulary neither layer knows. | **Layer 2 shipped 2026-07-09**: a semantic router (existing ONNX embedder, cosine vs per-device exemplars) understands paraphrases — "i'm melting in here" → fan (confident → acts; plausible → asks "Should I…?"). Remaining layers in FUTURE_IDEAS: GBNF-constrained tool calls + streaming tool support. Security invariant unchanged: every layer only *proposes*. |
| 2 | **LLM tool-calling is wired into `/inbox` only — the streaming web chat (`/chat/stream`) sends NO tools to the model**, and the 2B model's tool-calling is not fully reliable anyway (may answer in prose, or hallucinate). | In web chat, device commands the fast-paths don't recognize get a made-up text answer instead of an action. | Narrower than it sounds: the fast-paths (`intents.py`: volume, reminders, **home on/off/toggle/status**) run before the LLM on BOTH endpoints, and a device-shaped message that they and the semantic router both miss now gets a **tool round-trip** inside `_handle_home_command` — also on both endpoints. So the toolless streaming model is never handed a device command; it either acts, asks, or clarifies. What is still missing is tools during ordinary streamed conversation. Planned: streaming tool-call support + GBNF grammar-constrained output (FUTURE_IDEAS). |
...
| 4 | **Presence is a 180 s window and arrival state is in-memory.** Someone who leaves can read "present" for ~3 min; an orchestrator restart re-greets on next sighting. | Presence-gated control can lag reality; duplicate greetings after deploys. | Accepted for a household; the window is the `ttl_s` default on `memory.get_present_people` (`memory.py`, 180 s) — not in `config.py`, where this row used to send people looking. |
...
| 7 | **The server-box voice listener needs a mic attached to the server.** | No multi-room mics for the *always-on box listener*. | **The browser path is done (v3.2.0):** the web UI runs BOTH the wake word (openWakeWord via onnxruntime-web) and Whisper STT in the tab, so any device with a browser is a wake-word mic, on its own CPU, with no audio crossing the wire. What remains server-side is the headless `whisper-stream` → `voice_bridge.py` listener for a box with a mic plugged into it; putting that on the camera-agent devices is the edge-voice roadmap item. |
...
| 11 | **The four container images have never been built AND started.** A green Actions build is not evidence: both plausible failure modes of the non-root change — uid 10001 against a bind-mounted volume, and nginx binding 8080 — happen at container start-up, which a build never reaches. | Unknown whether the published images run. | Pull one and start it. There is no Docker on the author's box, so this needs the laptop. |
| 12 | **Wake phrases, the reactor HUD, the mic picker and the voice settings panel have never been seen or heard by a human.** They are shipped, unit-tested and lint-clean; nobody has used them. | Unknown whether they *feel* right, or work at all against a real microphone and real speech. | Open `/voice` on a machine with a mic and try them. |
