...
| 2 | **Tool-calling on a 2B model is not fully reliable** — the model sometimes answers in prose instead of emitting the tool call, especially for unusual phrasings. | A device command occasionally comes back as text instead of an action. | Deterministic intent fast-paths (`intents.py`) cover the common commands without the LLM. Planned: GBNF grammar-constrained output (see FUTURE_IDEAS). |
