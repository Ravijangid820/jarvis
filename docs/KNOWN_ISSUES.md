...
| 2a | **Device-command understanding is hardcoded** — the fast-paths match fixed verb lists/regexes (`intents.py`), so paraphrases and indirect requests ("it's boiling in here", "make it dark") don't act; they get a clarification or a plain LLM answer. Adding a phrasing means editing a regex. | Jarvis feels scripted rather than smart for anything off the known vocabulary. | Layered plan in FUTURE_IDEAS → "Dynamic intent understanding": semantic routing with the existing ONNX embedder + GBNF-constrained tool calls + streaming tool support. Security invariant unchanged: any layer only *proposes*; allowlist + gates + audit still decide. |
