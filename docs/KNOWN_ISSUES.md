...
| 7 | **The wake-word voice listener still runs on the server box** — its mic must be attached to the server. | No multi-room / remote wake-word mics. | **Partly resolved (unreleased)**: the *web UI* now transcribes in the browser (Whisper WASM, on the user's own device), so typed-or-spoken web chat needs no server mic and no server STT CPU. The always-listening wake-word path is still server-side; moving it to the camera-agent devices remains the edge-voice roadmap item. |
