...
# 1b) Browser-side model assets (STT failsafe, wake word, face) — fetched by the shared script so
#     the sources and SHA-256 pins live in exactly ONE place, shared with every Docker image and
#     the GitHub Pages build. Same env switches apply: SKIP_STT_MODEL / SKIP_WAKE_MODELS /
#     SKIP_FACE_MODELS. A local copy of the face models may already exist under camera/models on a
#     machine that ran the camera setup — hard-copy it rather than re-downloading 38 MB.
mkdir -p "$REPO/models/face"
for m in face_detection_yunet_2023mar.onnx face_recognition_sface_2021dec.onnx; do
  [ -f "$REPO/models/face/$m" ] || { [ -f "$REPO/camera/models/$m" ] && cp "$REPO/camera/models/$m" "$REPO/models/face/$m"; }
done
if bash "$REPO/src/scripts/fetch_browser_models.sh" "$REPO/models"; then
  ok "browser-side models (face + wake + STT failsafe) present + verified"
else
  warn "one or more browser-side bundles are incomplete — re-run this script (details above)"
fi
