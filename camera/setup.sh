#!/usr/bin/env bash
# Bootstrap the Jarvis camera agent on Linux/macOS/Raspberry Pi (run on the device, not the server).
set -euo pipefail
...
uv pip install --python "$CAMERA/.venv/bin/python" -r "$CAMERA/requirements.txt"
...
Setup done. Next:
  1. On the SERVER, mint a device key:
     ...
     and save the printed key to:  $CAMERA/config/agent.key
  2. Review $CAMERA/config/config.json (server.url, camera, which detectors are enabled).
  3. Try it:  cd "$CAMERA" && .venv/bin/python -m jarvis_camera.agent --dry-run
